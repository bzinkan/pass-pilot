...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Special endpoint to help users complete pending setup and get back to original school
  app.post('/api/debug/complete-pending-setup', async (req, res) => {
    try {
      const { email, newPassword, preferredSchoolId } = req.body;
      ...
      // If preferredSchoolId is provided, try to find that user first
      if (preferredSchoolId) {
        targetUser = users.find(u => u.schoolId === preferredSchoolId);
      }
      
      // If no preferred school or not found, look for pending users
      if (!targetUser) {
        targetUser = users.find(u => u.isFirstLogin || u.status === 'PENDING');
      }
      
      // If still no target, use the first user
      if (!targetUser) {
        targetUser = users[0];
      }
      ...
      // Update the user to complete setup
      const updatedUser = await storage.updateUser(targetUser.id, {
        password: hashedPassword,
        isFirstLogin: false,
        status: 'ACTIVE',
        isAdmin: true,  // Promote to admin to help recover access
        role: 'ADMIN'
      });

      // Delete any duplicate accounts in other schools (optional cleanup)
      for (const user of users) {
        if (user.id !== targetUser.id) {
          try {
            await storage.deleteUser(user.id);
          } catch (e) {
            console.log(`Could not delete duplicate user ${user.id}:`, e);
          }
        }
      }
      ...
      res.json({ 
        success: true,
        message: 'Successfully completed pending setup and promoted to admin', 
        user: {
          id: updatedUser.id, 
      ...
      console.error('Complete pending setup error:', error);
      res.status(500).json({ message: 'Failed to complete pending setup' });
    }
  });
